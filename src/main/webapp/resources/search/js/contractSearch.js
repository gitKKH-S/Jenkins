let INDEX_DOC="contractm";
let INDEX_FILE="contractf";
		...
		if (index == INDEX_DOC) {
			...
			if (schGbn_id == '0') {
				...
				pQuery += " OR unstored_FIELD_CONTENTS:("
						+ ReplaceStr(searchTrim(pQuery_tmp), " ", operator)
						+ ")";
				pQuery += " OR unstored_FIELD_WRITER:("
						+ ReplaceStr(searchTrim(pQuery_tmp), " ", operator)
						+ ")";
				pQuery += ")";
			}
			...
			if (index== INDEX_DOC){
				if (data.list[i].stored_ATTACH_LIST !== null
					...
					for (j = 0; j < attach_list.length - 1; j++) {
						...
						if(j==0){
							result_list += "<li>";
							result_list += "<a href=\"javascript:downFile('"+fileNm+"', '"+sfileNm+"','BBS')\">";
...
//							result_list += file_desc_list[j];
				...
				var sfile_nm = data.list[i].stored_SFILE_NM.substring(0, data.list[i].stored_SFILE_NM.lastIndexOf("."))
...
var pageOpen = function(url){
	window.open(url,"","scrollbars=yes, toolbar=no, width=1100, height=700");
}