			...
			<xsl:if test="./@curstate != 'delete'">
				...
				<strong>제<xsl:value-of select="@contno"/>조<xsl:if test="./@contsubno>0">의<xsl:value-of select="@contsubno"/></xsl:if>
					...
					<xsl:if test="./cont/@tag!=''">
								<span style="color:blue"><xsl:value-of select="./cont/@tag"/></span>
								</xsl:if>
					</span>
					<xsl:choose> 
					...
					<xsl:if test="@revisiontag!=''">
					<span style="color:blue;font-size:12pt;"><xsl:value-of select="@revisiontag"/></span>
					</xsl:if>
				</p>
				<br/>
			</xsl:if>
