...
<xsl:template match="jo">
	<xsl:for-each select=".">
	<xsl:if test="./@singugbn='sin' or ./@gbn='new'">
...
<xsl:template match="cont/font" mode="link">
...
<xsl:template match="image|cont/image" mode="link">
	<table cellpadding='0' cellspacing='0' border='0'  width="100%" valign="top">
	<tr><td>
	<p><img><xsl:attribute name="src">/kribblaw/data/image/<xsl:value-of select="./@src"/></xsl:attribute><xsl:attribute name="align"><xsl:value-of select="@align"/></xsl:attribute></img></p>
	</td></tr>
	</table>
</xsl:template>
...
<xsl:param name="param" select="."/>
<xsl:param name="contid2"/>
<xsl:param name="abc2"/>
<table  width="100%"  valign="top">
    <xsl:variable name="abc"><xsl:value-of select="$revcha"/></xsl:variable>                	                 	
    <xsl:for-each select="./jo[@singugbn='sin' or @gbn='new']">
        <xsl:if test="./@startcha=$abc">
        <xsl:variable name="contid1"><xsl:value-of select="./@contid"/></xsl:variable>
        <tr>
            <td width="50%" bgcolor="#f4f4f4" valign="top">
                <xsl:call-template name="left">
                    <xsl:with-param name="contid2" select="$contid1"/>
                    <xsl:with-param name="abc2" select="$abc"/>
                    <xsl:with-param name="param" select="."/>
                </xsl:call-template>		
            </td>
...
<xsl:param name="contid2"/>
<xsl:param name="abc2"/>
<xsl:for-each select="//jo[@singugbn='gu']">
        <xsl:if test="./@contid=$contid2">        
            <xsl:if test="./@singugbn='gu'">
                ...
                <span class="jocontent"> 
					<xsl:apply-templates select="cont/text()|cont/법링크|cont/링크|cont/image|cont/span/img|cont/img|cont/a5b5|cont/span/a5b5|cont/span|cont/font" mode="link"/>
