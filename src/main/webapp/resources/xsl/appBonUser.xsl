...
<xsl:template match="law">
	...
	<div class="innerbody" id="innerbody" style="overflow-y:scroll;height:310;padding:10px">
		<xsl:if test="$Topcont!=''"> 
			<div id="dan">
	...
	<!-- 내규 제개정 select box -->
			<xsl:element name="p"> 				
				...
				<!--xsl:element name="img">
					...
					<xsl:attribute name="onclick">revision('<xsl:value-of select="$bookid"/>');</xsl:attribute>
					<xsl:attribute name="src">/kribblaw/images/btn_22.gif</xsl:attribute>
	...
	<xsl:variable name="the_min">
		...
		<xsl:for-each select=".">
			...
			<xsl:element name="div">
				<xsl:attribute name="id"><xsl:value-of select="@contid"/></xsl:attribute>
				<xsl:element name="img">
					<xsl:attribute name="src">../../resources/images/common/icon_print.gif</xsl:attribute>
					<xsl:attribute name="align">absmiddle</xsl:attribute>
					<xsl:attribute name="class">histIcon</xsl:attribute>
					<xsl:attribute name="alt">조항 인쇄</xsl:attribute>
					<xsl:attribute name="onclick">printJo('<xsl:value-of select="$bookid"/>','<xsl:value-of select="@contid"/>','bon<xsl:value-of select="@contno"/><xsl:if test="./@contsubno>0">bu<xsl:value-of select="@contsubno"/></xsl:if>');</xsl:attribute>
				...
				<xsl:if test="count(./panList)!=0"> 
					<div class="pbon" style="padding:10px">
					☞관련 판례정보
					<ul>
				...
				<div id="hang"  class="pbon" style="padding:10px;">
					<a style="cursor:pointer;color:red;">
						<xsl:attribute name="onclick">fileView('<xsl:value-of select="@contid"/>')</xsl:attribute>
						☞ 관련 문서정보 확인
					</a>
				</div>
				</xsl:if>
