...
//	total : 전체 결과 갯수
function pageNav( funcName, pageNum, pageSize, total ,datacd)
{
	if( total < 1 )
		return "";
...
//		ret += navAnchor(funcName, (firstPage-1), "<img src='"+rootPath+"/jsp/lkms3/images/icon/before.gif' alt='이전페이지' align='absmiddle'  border=0>") + "\n";
		ret += navAnchor(funcName, (firstPage-1), "◀ ",datacd) + "\n";
	}
	...
	for( i=firstPage; i<=lastPage; i++ )
	{
		if( pageNum == i ){
			...
			ret += navAnchor(funcName, i, "" + i + "</a>",datacd) + "&nbsp;\n";
		else
		  ret += navAnchor(funcName, i, "" + i + "",datacd) + "&nbsp;\n";
	}
...
//		ret += navAnchor(funcName, totalPages, "<img src='"+rootPath+"/jsp/lkms3/images/icon/last.gif' alt='마지막'  align='absmiddle'  border=0>") + "\n";
		ret += navAnchor(funcName, (lastPage+1), " ▶",datacd) + "\n";
	}

	return ret;
}

function navAnchor( funcName, pageNo, anchorText ,datacd)
{
	return "<a href='javascript:" + funcName + "(" + pageNo+",\"" + datacd + "\")' class='innerBtn'>" + anchorText + "</a>";
}
