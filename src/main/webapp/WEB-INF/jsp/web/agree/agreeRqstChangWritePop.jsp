...
// 	String cnstn_rqst_emp_no = request.getAttribute("cnstn_rqst_emp_no")==null?"":request.getAttribute("cnstn_rqst_emp_no").toString();
	...
	String CVTN_RQST_EMP_NO = request.getAttribute("CVTN_RQST_EMP_NO")==null?"":request.getAttribute("CVTN_RQST_EMP_NO").toString();
	String srchTxt = request.getAttribute("srchTxt")==null?"":request.getAttribute("srchTxt").toString();
	
	System.out.println("22222" + CVTN_MNG_NO);
...
<%-- 				document.frm.action="<%=CONTEXTPATH%>/web/suit/searchBankPop.do"; --%>
	...
	function setState(userno, state) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/agree/setChrgEmpState.do",
			data:{
				"userno" : userno,
				"state" : state
			},
	...
	function setRqstEmp(userno, usernm, deptno, deptnm){
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/agree/agreeRqstChang.do",
			data:{
				userno : userno
				,usernm : usernm
				,deptno : deptno
...
%>
				<tr>
					...
					<td>
						<%
						if(!CVTN_RQST_EMP_NO.equals(emp_no)){
						%>
