...
<div class="subCA">
	<div class="subBtnW side">
		<div class="subBtnC left">
			<strong class="subTT">
			(<%=agreeMap.get("CVTN_DOC_NO")==null?"":agreeMap.get("CVTN_DOC_NO").toString()%>)&nbsp;
...
<%
			...
			if(GRPCD.indexOf("Y")>-1 || GRPCD.indexOf("A")>-1 || GRPCD.indexOf("N")>-1){
				...
				if("완료".equals(PRGRS_STTS_SE_NM)) {
					out.println("<a href=\"#none\" class=\"sBtn type1\" onclick=\"consultRole();\">열람권한관리</a>");
...
%>
	...
	<div class="hkkresult">
		<hr class="margin40">
		<strong class="subST">의회동의 필요여부 상세 내용</strong>
			<%
				if((GRPCD.indexOf("Y")>-1 || GRPCD.indexOf("A")>-1 || GRPCD.indexOf("N")>-1)){
			%>
			...
			%>
				...
				<%
					for(int f=0; f<agreeFileList.size(); f++) {
						...
						if ("VIEW3".equals(fileSe)) {
							if("작성중".equals(PRGRS_STTS_SE_NM)) {
				%>
						<div>
							<%=file.get("PHYS_FILE_NM").toString()%>
							<a href="#none" class="innerBtn" onclick='downFile("<%=file.get("PHYS_FILE_NM").toString()%>", "<%=file.get("SRVR_FILE_NM").toString()%>", "AGREE")' style="margin-left: 30px; height: 20px; line-height:18px;">다운로드</a>
							<a href="#none" class="innerBtn" onclick="makeDoc('3');" style="height: 20px; line-height:18px;">의뢰서수정</a>
						</div>
				<%
							} else {
				%>
...
%>
		<div class="subBtnW side">
			<div class="subBtnC left">
				<strong class="subTT">협약 의뢰 답변</strong>
			</div>
		</div>
		<div class="innerB">
<%
...
%>
			<table class="infoTable write" style="width:100%">
				...
				<tr>
					<th>의뢰하신 의회동의 필요여부 의뢰를 처리중입니다.<br/><br/>완료 된 후 답변을 확인하세요.</th>
				</tr>
			</table>
<%
		}
