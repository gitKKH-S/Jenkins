...
<%
	...
	String CVTN_MEMO_MNG_NO = request.getAttribute("CVTN_MEMO_MNG_NO")==null?"":request.getAttribute("CVTN_MEMO_MNG_NO").toString();
...
<script type="text/javascript">
	var CVTN_MEMO_MNG_NO  = "<%=CVTN_MEMO_MNG_NO%>";
	...
	function editDocInfo(){
		opener.memoWrite(CVTN_MEMO_MNG_NO);
		window.close();
	}
	...
	function delDocInfo(){
		if(confirm("첨부파일도 함께 삭제됩니다.\n삭제하시겠습니까?")){
			$.ajax({
				...
				data:{CVTN_MEMO_MNG_NO:CVTN_MEMO_MNG_NO, CVTN_MNG_NO:CVTN_MNG_NO},
				dataType:"json",
				async:false,
...
<%-- 	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/> --%>
	...
	<strong class="popTT"> 협약 메모 관리 <a href="#none"
...
%>
							<div class="selFileDiv" onclick="downFile('<%=files.get("DWNLD_FILE_NM").toString()%>', '<%=files.get("SRVR_FILE_NM").toString()%>', 'AGREE')">
								<%=files.get("DWNLD_FILE_NM").toString()%>
							</div>
<%
						}
					}
