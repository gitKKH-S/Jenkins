...
<script>
	...
	function reload(){
		document.frm.action = "<%=CONTEXTPATH%>/web/agree/goAgreeChrgCost.do";
		document.frm.submit();
	}
	
	//비용 승인여부 처리
	function amtAprv(CVTN_CST_MNG_NO, RVW_TKCG_EMP_NO, CVTN_MNG_NO, CVTN_CST_AMT, gbn) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/agree/updateChrgLawyerAmt.do",
			data:{
				CVTN_CST_MNG_NO:CVTN_CST_MNG_NO,
	...
	// 비용 지급일자 처리
	function amtAprvYmd(CVTN_CST_MNG_NO) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/agree/updateChrgLawyerAmt.do",
			data:{
	...
	// 비용 지급일자 처리
	function amtGiveYmd(CVTN_CST_MNG_NO) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/agree/updateChrgLawyerAmt.do",
			data:{
	...
	function chkAprvY() {
		...
		setTimeout(function() {
			...
			var CVTN_CST_MNG_NO_LIST = [];
			var RVW_TKCG_EMP_NO_LIST = [];
			var CVTN_MNG_NO_LIST = [];
			var CVTN_CST_AMT_LIST = [];
			...
			for (let i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].checked) {
					if ($("#CST_PRGRS_STTS_SE"+checkboxes[i].value).val() != "R" && $("#CST_PRGRS_STTS_SE"+checkboxes[i].value).val() != "Z") {
						...
						CVTN_CST_MNG_NO_LIST.push($("#CVTN_CST_MNG_NO"+checkboxes[i].value).val());
						RVW_TKCG_EMP_NO_LIST.push($("#RVW_TKCG_EMP_NO"+checkboxes[i].value).val());
						CVTN_MNG_NO_LIST.push($("#CVTN_MNG_NO"+checkboxes[i].value).val());
						CVTN_CST_AMT_LIST.push($("#CVTN_CST_AMT"+checkboxes[i].value).val());
					}
				}
			}
			...
			$.ajax({
				type:"POST",
				url:"${pageContext.request.contextPath}/web/agree/updateChrgLawyerAmtList.do",
				data:{
					CVTN_CST_MNG_NO_LIST : CVTN_CST_MNG_NO_LIST,
					RVW_TKCG_EMP_NO_LIST : RVW_TKCG_EMP_NO_LIST,
					CVTN_MNG_NO_LIST     : CVTN_MNG_NO_LIST,
					CVTN_CST_AMT_LIST    : CVTN_CST_AMT_LIST,
					CST_PRGRS_STTS_SE    : "A"
				},
				dataType:"json",
				async:false,
		...
		<div class="innerB">
			<div>
				...
				<%
					if(costList.size() > 0) {
						for (int i=0; i<costList.size(); i++) {
							...
							String CVTN_CST_AMT      = map.get("CVTN_CST_AMT")==null?"":map.get("CVTN_CST_AMT").toString();
							String CVTN_CST_MNG_NO   = map.get("CVTN_CST_MNG_NO")==null?"":map.get("CVTN_CST_MNG_NO").toString();
				...
				%>
					<tr>
						<td>
							<input type="hidden" name="CVTN_CST_MNG_NO<%=i%>"   id="CVTN_CST_MNG_NO<%=i%>"   value="<%=CVTN_CST_MNG_NO%>">
							<input type="hidden" name="CVTN_MNG_NO<%=i%>"       id="CVTN_MNG_NO<%=i%>"       value="<%=CVTN_MNG_NO%>">
							...
							<input type="hidden" name="CVTN_CST_AMT<%=i%>"      id="CVTN_CST_AMT<%=i%>"      value="<%=CVTN_CST_AMT%>">
						...
						<td><%=map.get("CVTN_TTL")==null?"":map.get("CVTN_TTL").toString()%></td>
						<td><%=map.get("LWYR_NM")==null?"":map.get("LWYR_NM").toString()%></td>
						<td style="text-align:center;"><%=map.get("VIEW_CVTN_CST_AMT")==null?"":map.get("VIEW_CVTN_CST_AMT").toString()%></td>
						...
						%>
							<a href="#none" class="innerBtn" onclick="amtAprv('<%=CVTN_CST_MNG_NO%>', '<%=RVW_TKCG_EMP_NO%>', '<%=CVTN_MNG_NO%>', '<%=CVTN_CST_AMT%>', 'A');">승인</a>
							<a href="#none" class="innerBtn" onclick="amtAprv('<%=CVTN_CST_MNG_NO%>', '<%=RVW_TKCG_EMP_NO%>', '<%=CVTN_MNG_NO%>', '<%=CVTN_CST_AMT%>', 'X');">보완요청</a>
						<%
						...
						%>
							...
							<a href="#none" class="innerBtn" onclick="amtAprvYmd('<%=CVTN_CST_MNG_NO%>');">저장</a>
						<%
						...
						%>
							...
							<a href="#none" class="innerBtn" onclick="amtGiveYmd('<%=CVTN_CST_MNG_NO%>');">저장</a>
						<%
							} else if ("A".equals(CST_PRGRS_STTS_SE) && !CST_GIVE_YMD.equals("")) {
						%>
							<a href="#none" class="innerBtn" onclick="amtAprv('<%=CVTN_CST_MNG_NO%>', '<%=RVW_TKCG_EMP_NO%>', '<%=CVTN_MNG_NO%>', '<%=CVTN_CST_AMT%>', 'C');">지급</a>
						<%
							}
						%>
