		...
		<div id="gridList">
			<div class="tableW">
				<form name="wform" method="post" enctype="post">
					<input type="hidden" name="BBS_SE" id="BBS_SE" value="CON"/>
					...
					<table class="infoTable write">
						...
						<tr>
							<th>관리번호</th>
							<td>
								<%=bon.get("DOC_MNG_NO")==null?"":bon.get("DOC_MNG_NO").toString()%>
							</td>
							<th>등록자</th>
							<td>
								<%=bon.get("WRTR_EMP_NM")%>
						...
						<tr>
							<th>회의건명</th>
							<td colspan="5">
								<%=bon.get("PST_TTL")==null?"":bon.get("PST_TTL").toString()%>
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="5">
								<div>
								<%=bon.get("RMRK_CN")==null?"":bon.get("RMRK_CN").toString().replaceAll("\n","<br/>")%>
								</div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="5">
								<div>
								<%=bon.get("PRCS_STTS_SE_NM")==null?"":bon.get("PRCS_STTS_SE_NM").toString().replaceAll("\n","<br/>")%>
								</div>
							</td>
						</tr>
						<tr>
							...
							<td colspan="5">
								<div>
								<%=bon.get("TKCG_EMP_NM")==null?"":bon.get("TKCG_EMP_NM").toString().replaceAll("\n","<br/>")%>
								</div>
							</td>
						</tr>
						<tr>
							...
							<td colspan="5">
								...
								<%
									for(int i=0; i<flist.size(); i++){
										HashMap result = (HashMap)flist.get(i);
								%>
									<li class="filename" onclick="downpage('<%=result.get("PHYS_FILE_NM") %>','<%=result.get("SRVR_FILE_NM") %>','CONSULT')" style="cursor:pointer;"><%=result.get("PHYS_FILE_NM") %></li>
								<%
									}
								%>
								</ul>
