...
<!--
	function gostr(id,one,tow,tree,four){
		if(id == 'A'){
			alert("현재기준입니다!!");
			return;	
		}else if(id =='B'){
			...
			document.chan.Book_id_r.value=tree;
			document.chan.Book_id_r2.value=four;
		}else if(id == 'C'){
			document.chan.Book_id_l.value=tree;
			document.chan.Book_id_c.value=tow;
			document.chan.Book_id_r.value=one;
			document.chan.Book_id_r2.value=four;
		}else if(id == 'D'){
			document.chan.Book_id_l.value=four;
			document.chan.Book_id_c.value=one;
			document.chan.Book_id_r.value=tow;
			document.chan.Book_id_r2.value=tree;
		}
		form = document.chan;
		form.id.value=id;
		
		form.action="./4_compare.do";
		form.submit();
	}
//-->
</script>
...
<%
	...
	String Book_id_r2 = ServletRequestUtils.getStringParameter(request,"Book_id_r2","");
	...
	int list_cnt_dan=DanList.size();
	String Obookid_l="";//왼쪽
	String Obookid_c="";//중앙
	String Obookid_r="";//오른쪽
	String Obookid_r2="";//오른쪽
	String Booktitle_l="";//왼쪽
	String Booktitle_c="";//중앙
	String Booktitle_r="";//오른쪽
	String Booktitle_r2="";//오른쪽
	String XmlData_l="";
	String XmlData_c="";
	String XmlData_r="";
	String XmlData_r2="";
	for(int k=0;k<list_cnt_dan;k++){
		...
		}else if(Bookid.equals(Book_id_r)){
			Obookid_r = bylaw.get("OBOOKID")==null?"":bylaw.get("OBOOKID").toString();
			Booktitle_r = bylaw.get("TITLE")==null?"":bylaw.get("TITLE").toString();
			XmlData_r = bylaw.get("XMLDATALINK")==null?"":bylaw.get("XMLDATALINK").toString();
		}else if(Bookid.equals(Book_id_r2)){
			Obookid_r2 = bylaw.get("OBOOKID")==null?"":bylaw.get("OBOOKID").toString();
			Booktitle_r2 = bylaw.get("TITLE")==null?"":bylaw.get("TITLE").toString();
			XmlData_r2 = bylaw.get("XMLDATALINK")==null?"":bylaw.get("XMLDATALINK").toString();
		}
	}
%>
<%!
...
<table width ="100%" style="border-top:2px solid #60AEE0;border-bottom:1px solid #60AEE0;" bgcolor="#D2E4FA">
	<tr>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='FFFFFF'" onmouseout="this.style.backgroundColor=''" onclick="gostr('A','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
		...
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='FFFFFF'" onmouseout="this.style.backgroundColor=''" onclick="gostr('B','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
		...
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='FFFFFF'" onmouseout="this.style.backgroundColor=''" onclick="gostr('C','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='<%=Booktitle_r %>' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"><%=Booktitle_r %></nobr>
			</span>
		</th>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='FFFFFF'" onmouseout="this.style.backgroundColor=''" onclick="gostr('D','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='<%=Booktitle_r2 %>' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"><%=Booktitle_r2 %></nobr>
			</span>
		</th>
	</tr>
</table>
<%	
	File xslfile = new File(XSLURL+"3_compare.xsl");
	...
	String Cont_cd1="";
	String Cont_no1="";
	String Contents_web1="";
	String Cont_subno1="";
	
	String Contents2="";
	String Cont_cd2="";
	String Cont_no2="";
	String Contents_web2="";
	String Cont_subno2="";
%>
...
out.println("<table border=\"0\" width=\"100%\">");
		...
		if(!Cont_cd.equals("본문")){
			...
			if(!Contents_web.equals("")){
				...
				for(int h=0; h<sList.size(); h++){
					...
					Contents1=sub.get("Contents")==null?"":la.get("Contents").toString();//제목받아오기	
					Cont_cd1=sub.get("Contcd")==null?"":la.get("Contcd").toString();
					Cont_no1=sub.get("Contno")==null?"":la.get("Contno").toString();
					Contents_web1=sub.get("Contentsweb")==null?"":la.get("Contentsweb").toString();
					Cont_subno1=sub.get("Contsubno")==null?"":la.get("Contsubno").toString();				
			...
			out.println("</span></td>");
			out.println("<td width=\"25%\" valign=\"top\">");
			if(!Contents_web1.equals("")){
				ArrayList sList = getScont(XmlData_r,Book_id_c,Cont_no1,Cont_subno1);
				for(int h=0; h<sList.size(); h++){
					HashMap sub = (HashMap)sList.get(h);
					Contents2=sub.get("Contents")==null?"":la.get("Contents").toString();//제목받아오기	
					Cont_cd2=sub.get("Contcd")==null?"":la.get("Contcd").toString();
					Cont_no2=sub.get("Contno")==null?"":la.get("Contno").toString();
					Contents_web2=sub.get("Contentsweb")==null?"":la.get("Contentsweb").toString();
					Cont_subno2=sub.get("Contsubno")==null?"":la.get("Contsubno").toString();			
			...
			out.println("</span></td>");
			out.println("<td width=\"25%\" valign=\"top\">");
			if(!Contents_web2.equals("")){
				ArrayList sList = getScont(XmlData_r2,Book_id_r,Cont_no2,Cont_subno2);
				for(int h=0; h<sList.size(); h++){
					HashMap sub = (HashMap)sList.get(h);
					if(sub.get("Contno")!=null&&!sub.get("Contno").toString().equals("")){
						String joTitle="<span class=jonumber>제"+sub.get("Contno")+"조("+sub.get("Contents")+")</span><br/><span class=jocontent>";
	...
	%>
	<tr>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='e0ffff'" onmouseout="this.style.backgroundColor=''" onclick="gostr('A','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"></nobr>
			</span>
		</th>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='e0ffff'" onmouseout="this.style.backgroundColor=''" onclick="gostr('B','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"></nobr>
			</span>
		</th>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='e0ffff'" onmouseout="this.style.backgroundColor=''" onclick="gostr('C','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"></nobr>
			</span>
		</th>
		<th width="25%" align="center" onMouseOver="this.style.backgroundColor='e0ffff'" onmouseout="this.style.backgroundColor=''" onclick="gostr('D','<%=Book_id_l %>','<%=Book_id_c %>','<%=Book_id_r %>','<%=Book_id_r2 %>');">
			<span title='' class="aaa"  style="cursor:hand;">
				<nobr style="text-overflow:ellipsis;overflow:hidden;width=300PX"></nobr>
			</span>
		</th>
