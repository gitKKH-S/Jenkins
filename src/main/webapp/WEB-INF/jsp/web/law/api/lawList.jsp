...
<script type="text/javascript">
	...
	Ext.onReady(function(){
		...
		var jbAry = new Array();
		chkcmn = new Ext.grid.CheckboxSelectionModel({
			singleSelect: false,
			listeners:{
				selectionchange:function(){
					jbAry = new Array();
					var sels = chkcmn.getSelections();
					if (sels.length > 0) {
						Ext.each(sels, function(sel) {
							jbAry.push(sel.get('lawid'));
							console.log("===>"+jbAry);
						});
					}
				},
		...
		var cmm = new xg.ColumnModel({
				columns:[
						...
						{header:"<b>법령명</b>",  style: 'text-align:center', dataIndex:'title', sortable: true},
						{header:"<b>소관부처</b>",  width:30, align:'center', dataIndex:'dept'},
						{header:"<b>제정·개정구분</b>",  width:40, align:'center', dataIndex:'revcd'},
						{header:"<b>법령종류</b>",  width:40, align:'center', dataIndex:'lawcd'},
						{header:"<b>공포번호</b>",  width:40, align:'center', dataIndex:'promulno', sortable: true},
						{header:"<b>공포일</b>",  width:40, align:'center', dataIndex:'promuldt', sortable: true},
						{header:"<b>시행일</b>",  width:40, align:'center', dataIndex:'startdt', sortable: true}
					]
				});
		gridCodeMan = new xg.GridPanel({
			...
			listeners: {
				...
				cellclick: function(grid, iCellEl, iColIdx, iStore, iRowEl, iRowIdx, iEvent) {
					if(iColIdx!=0){
						var selModel=grid.getSelectionModel();
						selModel.selectRow(iRowIdx);
						var rowData = selModel.getSelected();
						console.log(rowData.get("dlink"));
...
<!-- 			<a href="#none" id="favBtn" class="sBtn type2">즐겨찾기 등록</a> -->
		</div>
	</div>
	<div class="innerB">
		<div id="gridList"></div>
