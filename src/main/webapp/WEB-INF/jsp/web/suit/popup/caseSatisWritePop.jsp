...
<script type="text/javascript">
	...
	function satisSave(){
		...
		if(lawyerListCnt == 0){
			return alert("사건위임 탭에서 변호사를 등록 하세요");
		}
...
%>
				...
				if(satisItemListCnt > j){
					...
					satisDataTmp.TRGT_PST_MNG_NO = frmTmp.INST_MNG_NO.value;
...
<form id="frm" name="frm" method="post" action="">
	...
	<strong class="popTT"> 소송 만족도 평가 <a href="#none"
...
%>
					<li id="tabBtn<%=i%>">
						<a href="#" onclick="tabBtn_onclick(<%=i%>)" style="padding:15px 15px;"><%=lawinfo.get("JDAF_CORP_NM")%></a>
					</li>
<%
				}
%>
...
%>
				<div class="tableW" id="satisItems<%=i%>">
					<form id="satisFrm<%=i%>" name="satisFrm<%=i%>" method="post">
						<input type="hidden" id="LWS_MNG_NO"              name="LWS_MNG_NO"              value="<%=LWS_MNG_NO%>" />
						<input type="hidden" id="INST_MNG_NO"             name="INST_MNG_NO"             value="<%=INST_MNG_NO%>" />
						<input type="hidden" id="WRTR_EMP_NM"                  name="WRTR_EMP_NM"                  value="<%=USERNO%>" />
						<input type="hidden" id="DGSTFN_SRVY_TRGT_MNG_NO" name="DGSTFN_SRVY_TRGT_MNG_NO" value="<%=lawinfo.get("AGT_MNG_NO")%>" />
						<input type="hidden" id="JDAF_CORP_NM"            name="JDAF_CORP_NM"            value="<%=lawinfo.get("JDAF_CORP_NM")%>" />
						<input type="hidden" id="TRGT_PST_MNG_NO"         name="TRGT_PST_MNG_NO"         value="<%=INST_MNG_NO%>" />
...
%>
				...
				<div id="nonDiv">
					<br/>
					사건 위임에 등록 된 법무법인이 없습니다.
					<br/>
					사건 위임 탭에서 변호사를 등록 해 주세요
				</div>
<%
			}
%>
