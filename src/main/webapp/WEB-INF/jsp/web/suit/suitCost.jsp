...
<script>
	...
	function reload(){
		document.frm.action = "<%=CONTEXTPATH%>/web/suit/goSuitCost.do";
		document.frm.submit();
	}
	...
	// 비용 지급여부 처리
	function amtGive(CST_MNG_NO, LWS_MNG_NO, INST_MNG_NO, gbn, APRV_YN) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/suit/setOutCostState.do",
			data:{
	...
	// 비용 지급일자 처리
	function amtGiveYmd(CST_MNG_NO, LWS_MNG_NO, INST_MNG_NO, APRV_YN) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/suit/setOutCostState.do",
			data:{
	...
	function chkAprvY() {
		...
		setTimeout(function() {
			...
			for (let i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].checked) {
					if ($("#APRV_YN"+checkboxes[i].value).val() == "N" 
						...
						CST_MNG_NO_LIST.push($("#CST_MNG_NO"+checkboxes[i].value).val());
					}
				}
			}
			...
			$.ajax({
				type:"POST",
				url:"${pageContext.request.contextPath}/web/suit/updateCostAmtList.do",
				data:{
					LWS_MNG_NO_LIST:LWS_MNG_NO_LIST,
					INST_MNG_NO_LIST:INST_MNG_NO_LIST,
					CST_MNG_NO_LIST:CST_MNG_NO_LIST,
					APRV_YN:"Y"
				},
				dataType:"json",
				async:false,
...
<form name="frm" id="frm" method="post" action="">
	...
	<div class="subCA">
		...
		<div class="subBtnW side" style="float:right; margin-top:10px">
			...
			<%if(GRPCD.indexOf("Y") > -1 || GRPCD.indexOf("B") > -1) {%>
		...
		<div class="innerB">
			<div>
				<div id="subTitle"><strong class="countT">소송비용 신청 목록</strong></div>
				<hr class="margin10">
				...
				<%
					if(costList.size() > 0) {
						for (int i=0; i<costList.size(); i++) {
							...
							String CST_MNG_NO = map.get("CST_MNG_NO")==null?"":map.get("CST_MNG_NO").toString();
				...
				%>
					<tr>
						<td>
							<input type="hidden" name="CST_MNG_NO<%=i%>"  id="CST_MNG_NO<%=i%>"  value="<%=CST_MNG_NO%>">
						...
						<td style="text-align:center;"><%=map.get("CST_SE_NM")==null?"":map.get("CST_SE_NM").toString()%></td>
						<td style="text-align:center;"><%=map.get("PRCS_AMT")==null?"":map.get("PRCS_AMT").toString()%></td>
						...
						<%if(GRPCD.indexOf("Y") > -1 || GRPCD.indexOf("B") > -1) {%>
							...
							<a href="#none" class="innerBtn" onclick="amtGiveYmd('<%=CST_MNG_NO%>', '<%=LWS_MNG_NO%>', '<%=INST_MNG_NO%>', '<%=APRV_YN%>');">저장</a>
							<%} else if ("Y".equals(APRV_YN) && !GIVE_YMD.equals("")) {%>
							<a href="#none" class="innerBtn" onclick="amtGive('<%=CST_MNG_NO%>', '<%=LWS_MNG_NO%>', '<%=INST_MNG_NO%>', 'Y', 'Y');">지급</a>
							<%} else {%>
								-
							<%}%>
						<%}%>
				...
				%>
					<tr>
						<td colspan="10">처리중인 소송비용 정보가 없습니다.</td>
					</tr>
				<%
					}
				%>
