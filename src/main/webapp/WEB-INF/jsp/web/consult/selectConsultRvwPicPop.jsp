...
<%
	...
	String inoutcon = request.getParameter("inoutcon")==null?"I":request.getParameter("inoutcon");
	...
	System.out.println("inoutcon??? " + inoutcon);
%>
<script language="javascript" src="${resourceUrl}/js/mten.pagenav.js"></script>
<script src="${resourceUrl}/js/mten.static.js"></script>
<%-- <link rel="stylesheet" type="text/css" href="${resourceUrl}/seoul/css3/common.css"> --%>
<%-- <link rel="stylesheet" type="text/css" href="${resourceUrl}/seoul/css3/layout.css"> --%>
<%-- <link rel="stylesheet" type="text/css" href="${resourceUrl}/seoul/css3/sub.css"> --%>
...
<script type="text/javascript">
	...
	var cnt = "<%=consultLawyerList.size()%>";
...
// 			$(".lawli").remove();
	...
	function saveConsultLawyer() {
// 		var inoutcons = $('input:radio[name="inoutconTmp"]:checked').val();
		
// 		$("#inoutcon").val($('input:radio[name="inoutconTmp"]:checked').val())
		...
		$("#inoutcon").val(inoutcon);
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/consult/consultLawInfoSave.do",
...
// 			},
	...
	function deleteConsultLawyer(consultLawyerid) {
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/consult/deleteConsultLawyer.do",
			data:{
...
// 			},
			...
			success:function(result){
				alert("삭제가 완료되었습니다.");
				opener.viewReload();
				document.frm.action="<%=CONTEXTPATH%>/web/consult/selectConsultRvwPicPop.do";
				document.frm.submit();
			},
			error:function(request, status, error){
				alert("저장에 실패하였습니다. 관리자에게 문의바랍니다.");
	...
	function setChrg(userno, usernm){
		$.ajax({
			type:"POST",
			url:"${pageContext.request.contextPath}/web/consult/setConsultRvwPic.do",
			data:{
				"userno" : userno,
				"usernm" : usernm,
				"consultid" : "<%=consultid%>"
	...
	<div style="height:74%">
		<div class="popC">
			<div class="popSrchW">
				...
				<input type="text" id="office" name="office" value="<%=office%>" style="width:720px;" placeholder="검색 할 변호사명을 입력해주세요">
				<a href="#none" class="sBtn type1" onclick="javascript:searchLaw();" id="schBtn"><i class="fa-solid fa-magnifying-glass"></i> 검색</a>
				<a href="#none" class="sBtn type3" onclick="javascript:saveConsultLawyer();"><i class="fa-regular fa-floppy-disk"></i> 저장</a>
			</div>
			<!-- 안내 문구 영역 -->
			<div style="margin-bottom:10px; font-size: 13px; color: #007BFF; line-height: 1.6;">
				<ul style="list-style: disc; padding-left: 20px; margin: 0;">
					<li>* 평균답변일, 평균만족도는 전체, 5건, 10건 중 선택한 기간의 통계가 표시됩니다.</li>
					<li>* 소송, 자문, 협약란에 표시되는 건수는 현재 진행중인 건 수 입니다.</li>
					<li>* 변호사명 클릭 시, 상세 정보가 팝업으로 표시됩니다. 변호사명 우측의 [선택]을 클릭하여 선택해주세요.</li>
				</ul>
			</div>
...
<!-- 							<td align="center"> -->
<%-- 								<input type="radio" name="inoutconTmp" value="I" <%if ("I".equals(inoutcon) || inoutcon.equals("")){out.print("checked");} %> onclick="javascript:sendInoutcon(this.value);"/> 내부검토 &nbsp;&nbsp;&nbsp;&nbsp; --%>
<%-- 								<input type="radio" name="inoutconTmp" value="O" <%if ("O".equals(inoutcon)){out.print("checked");} %> onclick="javascript:sendInoutcon(this.value);"/> 외부자문<!-- &nbsp;&nbsp;&nbsp;&nbsp; --> --%>
<!-- 							</td> -->
<!-- 						</tr>	 -->
						<tr>
							...
							%>
									<li class='lawli' id='law<%=i%>'>
										...
										<%}%>
										&nbsp;
										<a href='#none' class='innerBtn law' onclick="deleteConsultLawyer('<%=consult.get("RVW_TKCG_MNG_NO")==null?"":consult.get("RVW_TKCG_MNG_NO").toString()%>');">삭제</a>
									</li>
									&nbsp;&nbsp;&nbsp;
							<%
									}
					...
					<table class="infoTable pop" style="border-top:2px solid #000; width:100%;">
						...
						%>
							<tr id="lawTr<%=lawyer.get("LWYR_MNG_NO")%>">
								...
								<td style="text-align:center;"><%=lawyer.get("ARSP_NM")==null?"&nbsp":lawyer.get("ARSP_NM") %></td>
