...
// 	String consultid = request.getAttribute("consultid")==null?"":request.getAttribute("consultid").toString();
	...
	String inoutcon = request.getAttribute("inoutcon")==null?"":request.getAttribute("inoutcon").toString();

	String checkVal = request.getAttribute("checkVal")==null?"":request.getAttribute("checkVal").toString();
	
	HashMap reviewComment = request.getAttribute("reviewComment")==null?new HashMap():(HashMap)request.getAttribute("reviewComment");
	List rcfilelist = request.getAttribute("rcfilelist")==null?new ArrayList():(ArrayList)request.getAttribute("rcfilelist");
	
// 	String progid = memoMap.get("PROGID")==null?"":memoMap.get("PROGID").toString();
...
<script type="text/javascript">
	...
	function saveProgInfo(sfilenm, vfilenm){
		...
		if(confirm("내용을 저장하시겠습니까?")){
			...
			if ($("#CNSTN_ANS_RVW_OPNN_CN").val() == "") {
				return alert("검토 의견을 입력하세요.");
			}
			
			var frm = document.wform;
			$.ajax({
				type:"POST",
				url : "${pageContext.request.contextPath}/web/consult/saveReviewComment.do",
...
// 				},
				...
				success : function(result){
					if(result.data.msg =='ok'){
						$("#CNSTN_MNG_NO").val(result.data.CNSTN_MNG_NO);
...
<%-- 	<input type="hidden" name="consultid" id="consultid" value="<%=consultid%>"/> --%>
	...
	<input type="hidden" name="gbnid"    id="gbnid"    value=""/>
	<input type="hidden" name="filegbn"   id="filegbn"   value="ANSWERCOMMENT">
...
<!-- 				</tr> -->
				<tr>
					<th>검토 의견</th>
					<td>
						<textarea id=CNSTN_ANS_RVW_OPNN_CN name="CNSTN_ANS_RVW_OPNN_CN" rows="10" cols=""><%=reviewComment.get("CNSTN_ANS_RVW_OPNN_CN")==null?"":reviewComment.get("CNSTN_ANS_RVW_OPNN_CN").toString()%></textarea>
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<div id="fileUpload" class="dragAndDropDiv" <%if(rcfilelist.size()>0){ %>style="width:50%"<%} %>>
							<input type="file" multiple style="display:none" id="filesel"/>
							<label for="filesel"><strong>업로드 할 파일을 선택 하세요</strong></label>
<!-- 							<div><strong>업로드 할 파일을 선택 하세요</strong></div> -->
						</div>
						<div id="fileList">
							<input type="hidden" />
						</div>
						...
						<div class="hkk2" >
							<%
								for(int i=0; i<rcfilelist.size(); i++){
									HashMap result = (HashMap)rcfilelist.get(i);
