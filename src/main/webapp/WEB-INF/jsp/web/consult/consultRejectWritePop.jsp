...
<%
	...
	String stateCd = request.getAttribute("stateCd")==null?"":request.getAttribute("stateCd").toString();
...
// 				},
				...
				success : function(result){
					if(result.data.msg =='ok'){
						$("#memoid").val(result.data.memoid);
						
						for (var i = 0; i < fileList.length; i++) {
							var formData = new FormData();
							formData.append('file'+i, fileList[i]);
							formData.append('gbnid', result.data.memoid);
							
							var other_data = $('#wform').serializeArray();
							$.each(other_data,function(key,input){
								if(input.name != 'memoid'){
...
// 								},
						...
						alert("저장되었습니다.");
						opener.consultStateChange(stateCd);
						window.close();
					}else{
...
<%-- 	<input type="hidden" name="${_csrf.parameterName}"   value="${_csrf.token}"/> --%>
	...
	<input type="hidden" name="memoid"    id="memoid"    value="<%=cnstn_memo_mng_no%>"/>
	<input type="hidden" name="filegbn"   id="filegbn"   value="CREJECT">
