...
%>
				...
				if(satisItemListCnt > j){
					...
					satisDataTmp.TRGT_PST_MNG_NO = frmTmp.CNSTN_MNG_NO.value;
		...
		if(chked){
			...
			$.ajax({
				url:"<%=CONTEXTPATH%>/web/consult/insertSatisfaction.do",
...
%>
				<div class="tableW" id="satisItems<%=i%>">
					<form id="satisFrm<%=i%>" name="satisFrm<%=i%>" method="post">
						<input type="hidden" id="CNSTN_MNG_NO"              name="CNSTN_MNG_NO"            value="<%=CNSTN_MNG_NO%>" />
						...
						<input type="hidden" id="TRGT_PST_MNG_NO"           name="TRGT_PST_MNG_NO"         value="<%=CNSTN_MNG_NO%>" />
