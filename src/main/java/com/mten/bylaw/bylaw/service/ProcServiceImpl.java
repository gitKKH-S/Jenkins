...
public class ProcServiceImpl implements ProcService {
	protected final static Logger logger = Logger.getLogger( ProcServiceImpl.class );
	...
	public String getProcessListButton(Map<String, Object> mtenMap) {
		...
		List hlist = commonDao.selectList("settingSql.getProcessListButton", mtenMap);
		String html="<div class=\"orderViewW\"><ul class=\"orderView\">";
		
		for(int i=0; i<hlist.size(); i++){
			HashMap re = (HashMap)hlist.get(i);
			String ilcls = "";
			if(i==hlist.size()-1){
				//ilcls = "end";
			}
			...
			html = html + "<li class=\""+ilcls+"\" onclick=\"reStore('"+re.get("STATEID")+"',this)\" style=\"cursor:pointer\">"+re.get("STATECD")+"</li>";
		}
		html = html + "</ul></div>";
		return html;
	}
	...
	public JSONObject getButtonList(Map<String, Object> mtenMap) {
		List blist = commonDao.selectList("settingSql.getButtonList", mtenMap);
